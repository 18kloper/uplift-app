...
async function resolveChannelId(token) {
  ...
  for (let page = 0; page < 5; page++) {
    ...
    if (hit) { cachedChannelId = hit.id; return hit.id; }
...
function formatLine(slug, weekNum, fieldKey, value, question) {
  ...
  if (fieldKey === "bot_chat") {
    const a = String(value ?? "").slice(0, 280);
    return `💬 *${who}*${testTag} asked the portal bot:\n> ${String(question || "").slice(0, 280)}\n🤖 "${a}"`;
  }
...
export async function postPortalInput({ slug, weekNum, fieldKey, value, question }) {
  try {
    const token = process.env.SLACK_BOT_TOKEN;
    if (!token) return;
    const text = formatLine(slug, weekNum, fieldKey, value, question);
    if (!text) return;
    const channel = await resolveChannelId(token);
    if (!channel) return;

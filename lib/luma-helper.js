...
export async function ensureLumaAttendanceSheet(sheets, spreadsheetId) {
  ...
  if (!sheetNames.includes(LUMA_ATTENDANCE_SHEET)) {
    ...
    // Tab exists — check header completeness against LUMA_ATTENDANCE_HEADERS
    const headerRes = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: `${LUMA_ATTENDANCE_SHEET}!1:1`,
    });
    const existingHeaders = (headerRes.data.values || [[]])[0] || [];
    if (!existingHeaders.includes("joinedAt")) {
      if (existingHeaders.includes("reviewStatus")) {
        // Older migration appended reviewStatus/reviewedAt directly after
        // rawStatus, skipping joinedAt — insert it in place so header labels
        // match the columns logLumaAttendance/approveAttendance actually write.
        const reviewStatusIdx = existingHeaders.indexOf("reviewStatus");
        const startLetter = colIndexToLetter(reviewStatusIdx);
        const endLetter = colIndexToLetter(reviewStatusIdx + 2);
        await sheets.spreadsheets.values.update({
          spreadsheetId,
          range: `${LUMA_ATTENDANCE_SHEET}!${startLetter}1:${endLetter}1`,
          valueInputOption: "USER_ENTERED",
          requestBody: { values: [["joinedAt", "reviewStatus", "reviewedAt"]] },
        });
      } else {
        // Neither column present — append fresh

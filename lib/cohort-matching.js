...
// The cap, expressed where the solver can actually act on it.
//
// A per-mentor session budget is a knapsack, and an assignment problem cannot
// hold one: whether a mentor has room for a second founder depends on which
// founder they got first. Enforcing it afterwards by taking slots away was
// worse than useless — it left eight founders with nobody at all.
//
// So the cap becomes a property of the slot instead. A mentor's Nth founder
// can expect roughly budget/N sessions, and what matters is how far that falls
// short of what the founder asked for:
//
//   7-10 mentor, first founder wants 7-10   ->  10 each, no shortfall
//   7-10 mentor, second founder wants 7-10  ->   5 each, five short  (penalised)
//   7-10 mentor, second founder wants 3     ->   5 each, no shortfall (free)
//   4-6 mentor, second founder wants 3      ->   3 each, no shortfall (free)
//   4-6 mentor, second founder wants 4-6    ->   3 each, three short (penalised)
//
// which is exactly the rule: a second founder only where the sessions are
// genuinely spare. Priced near a grade of match quality, because being
// stretched thin costs a founder about as much as a worse-fitting mentor. It
// stays a price rather than a prohibition, so when there are not enough
// mentor-hours in the pool the solver still matches everybody and the strain
// is reported instead of hidden as an unmatched founder.
const SHORTFALL_COST = 12;

export function sessionShortfall(mentee, mentor, extra = 0) {
  const share = sessionsFor(mentor.tier) / (extra + 1);
  return Math.max(0, sessionsFor(mentee.tier) - share);
}
...
export function buildCohort({ mentees, mentors, oneEach = true }) {
  ...
  const shortfall = waiting.map(a => slots.map(s => sessionShortfall(a, s.mentor, s.extra)));
  ...
  return {
    waiting, pool, slots, slotsOpen: slots.length, scores, blocked, waste, shortfall,
...
function summarise(pairs, waitingCount, ctx = null) {
  ...
  // Founders who will get fewer sessions than they asked for, and by how much
  // in total. With the pool short on mentor-hours this is the number that
  // matters: it says how much of the gap the cohort is absorbing.
  const short = pairs.map(p => ({ p, miss: sessionShortfall(p.mentee, p.mentor, p.extra || 0) })).filter(x => x.miss > 0);
  const shortSessions = Math.round(short.reduce((sum, x) => sum + x.miss, 0));
  return {
    ...
    shortchanged: short.length, shortSessions,
  ...
  const cost = rows.map(i => {
    ...
    for (let c = 0; c < m; c++) {
      ...
      row[c] = ctx.bestUtility - (
        ...
        - ctx.shortfall[i][j] * SHORTFALL_COST
      );
...
export function greedyPlan(ctx) {
  ...
  const pairs = [];
  // A hand-picker respects the session cap too, or the comparison flatters
  // the cohort plan for obeying a rule the baseline was never given.
  const hasRoom = (a, mentor) => {
    const ask = Math.min(sessionsFor(a.tier), sessionsFor(mentor.tier));
    const spent = (sessionsUsed.get(mentor.id) || 0) + committedSessions(mentor);
    return spent === 0 || spent + ask <= sessionsFor(mentor.tier);
  };
  ctx.waiting.forEach((a, i) => {
    ...
    for (const onlyFresh of ctx.oneEach ? [true, false] : [false]) {
      ...
      sessionsUsed.set(best.mentor.id, (sessionsUsed.get(best.mentor.id) || 0)
        + Math.min(sessionsFor(best.mentee.tier), sessionsFor(best.mentor.tier)));
      pairs.push(best);
      return;
    }
  });

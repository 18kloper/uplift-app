...
function keywordsOf(list) {
  return keywordsOfText((Array.isArray(list) ? list : [list]).filter(Boolean).join(" "));
}

// Shared with the peer-group builder, which needs the same vocabulary to tell
// what a founder brings from what another one is asking for.
export function keywordsOfText(text) {
  const t = String(text || "").toLowerCase();
  return FOCUS_KEYWORDS.filter(k => t.includes(k));
}
...
// The top threshold moved from 11 to 13 when depth alignment started paying up
// to two extra points. Without that, a pairing scored Perfect for the same fit
// that read Excellent the day before, and over half the cohort came out
// Perfect — the label stops carrying information at that point. Raising it by
// exactly the size of the new bonus keeps the grades meaning what they meant,
// so the depth signal changes who gets matched with whom rather than quietly
// promoting everybody.
export function gradeOf(score) {
  if (score >= 13) return { label: "Perfect match", short: "Perfect", bg: "#e8f8f0", color: "#1a6e42", tier: 4 };
...
export function scoreMentor(mentee, mentor) {
  ...
  if (mb && rb) {
    if (mb === rb) {
      // Alignment is worth more the deeper it goes. Two people who both signed
      // up for 7-10 sessions are agreeing to a materially different
      // relationship than two who both want the minimum, and that mutual
      // commitment is itself a sign of a strong match — so the bonus scales
      // with the tier rather than being flat across all three.
      score += 4 + (mb - 1);
      reasons.push(mb === 3
        ? "both committed to 7-10 sessions, the deepest tier"
        : `sessions aligned (both ${mb === 2 ? "4-6" : "3"})`);
    }
...
// Two founders is the ceiling for everybody, including the 7-10 mentors whose
// session budget would arithmetically stretch to three. Somebody who
// volunteered for a deep 7-10 relationship did not volunteer to run three
// shallow ones, and the sessions adding up is not the same as the offer being
// honoured.
const MAX_MENTEES = 2;
...
export function buildCohort({ mentees, mentors, oneEach = true }) {
  ...
  const eligible = mentors.filter(isEligibleMentor);
  const singleSupply = eligible.reduce((n, mt) => n + slotsFor(mt, false), 0);
  const allowMultiple = singleSupply < waiting.length;
  // A mentor already carrying their allowance is not part of this problem.
  // Leaving them in the pool is how thirty-three mentors who each already have
  // a founder came to be reported as "waiting for a founder" the moment the
  // waiting room shrank to four: they have no slot to fill, so they can be
  // neither used nor idle.
  const pool = eligible.filter(mt => slotsFor(mt, allowMultiple) > 0);
  const alreadyPlaced = eligible.length - pool.length;
  ...
  return {
    waiting, pool, eligible, alreadyPlaced, slots, slotsOpen: slots.length, scores, blocked, waste, gap,
...
//
// The bonus has to be earned, though. "Every mentor gets one" is there so
// nobody who volunteered is told there was no founder for them — it is not a
// reason to park a founder on a weak match. Where the only founder a mentor
// could take is a weak fit, the bonus lapses and that founder goes to somebody
// they actually suit, even if it means the mentor waits. A weak pairing still
// beats no mentor at all, so nobody is left unmatched by this.
function loadBonus(ctx, extra, tier = 1) {
...
function summarise(pairs, waitingCount, ctx = null) {
  ...
  const load = ctx ? pairs.reduce((sum, p) => sum + loadBonus(ctx, p.extra || 0, gradeOf(p.score).tier), 0) : 0;
  ...
  const cost = rows.map(i => {
    ...
    for (let c = 0; c < m; c++) {
      ...
      row[c] = ctx.bestUtility - (
        ...
        + loadBonus(ctx, ctx.slots[j].extra, gradeOf(ctx.scores[i][j].score).tier)
...
function planFrom(ctx, extraPairs, skipRows, skipSlots, prefer) {
  ...
  // A mentor left waiting is something to act on, not just a number: either
  // their application is too thin to match on, or the cohort genuinely has
  // nobody who suits them. Name them and say which.
  const used = new Set(all.map(p => p.mentor.id));
  // Only a mentor with NO founder at all is a problem worth naming. One who
  // already has somebody and merely has room for a second is not waiting for
  // anything, and listing them as such is what made this panel unreadable.
  const idle = ctx.pool.filter(mt => !used.has(mt.id) && !(mt.assignedTo || []).length).map(mt => {
    const best = ctx.waiting
      .map(a => ({ mentee: a, score: scoreMentor(a, mt).score }))
      .sort((x, y) => y.score - x.score)[0] || null;
    return {
      mentor: mt,
      best,
      thinApplication: !(mt.focusAreas || []).length || !(mt.stagePref || []).length,
    };
  });
  return {
    pairs: all,

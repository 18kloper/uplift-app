...
const SECOND_MENTEE_PENALTY = 12;             // spread the load before stacking it

export function utilityOf(score) {
  return TIER_UTILITY[gradeOf(score).tier] + score * 0.1;
}

// How many more founders this mentor can take. Their own session availability
// is the ceiling: somebody offering three sessions total cannot carry three
// founders. Anyone already carrying a founder has that subtracted.
export function slotsFor(mentor, allowMultiple) {
  const taken = (mentor.assignedTo || []).length;
  const ceiling = allowMultiple ? Math.max(1, tierBand(mentor.tier) || 1) : 1;
...
export function buildCohort({ mentees, mentors }) {
  ...
  const bestUtility = Math.max(0, ...scores.flat().map(x => utilityOf(x.score)));

  return { waiting, pool, slots, slotsOpen: slots.length, scores, blocked, allowMultiple, bestUtility };
}

function summarise(pairs, waitingCount) {
...
function solve(ctx, skipRows = new Set(), skipSlots = new Set(), prefer = null) {
  ...
  const cost = rows.map(i => {
    ...
    for (let c = 0; c < m; c++) {
      ...
      const j = cols[c];
      if (ctx.blocked[i][j]) { row[c] = ctx.bestUtility - NO_MATCH_UTILITY + 1000; continue; }
      row[c] = ctx.bestUtility - (utilityOf(ctx.scores[i][j].score) - ctx.slots[j].extra * SECOND_MENTEE_PENALTY);
  ...
  for (let r = 0; r < n; r++) {
    ...
    const j = cols[c];
    if (ctx.blocked[i][j]) { unassigned.push(ctx.waiting[i]); continue; }
    pairs.push({
...
function planFrom(ctx, extraPairs, skipRows, skipSlots, prefer) {
  const { pairs, unassigned } = solve(ctx, skipRows, skipSlots, prefer);
  const all = [...extraPairs, ...pairs];
  return {
    pairs: all,
    byMentee: Object.fromEntries(all.map(p => [p.mentee.id, p])),
    unassigned,
    summary: summarise(all, ctx.waiting.length),
  };
}
...
export function greedyPlan(ctx) {
  ...
  ctx.waiting.forEach((a, i) => {
    ...
    ctx.slots.forEach((s, j) => {
      ...
      if (!best || sc > best.score) best = { mentee: a, mentor: s.mentor, score: sc, reasons: ctx.scores[i][j].reasons };
    });
    if (!best) return;
    left.set(best.mentor.id, left.get(best.mentor.id) - 1);
    pairs.push(best);
  });
  return { pairs, byMentee: Object.fromEntries(pairs.map(p => [p.mentee.id, p])), summary: summarise(pairs, ctx.waiting.length) };
}
...
export function recommendFor(ctx, menteeId, top = 3) {
  ...
  for (const [mentorId, j] of firstSlot) {
    ...
    candidates.push({
      ...
      cohortUtility: plan.summary.utility,
  ...
  candidates.sort((a, b) => b.cohortUtility - a.cohortUtility || b.score - a.score);

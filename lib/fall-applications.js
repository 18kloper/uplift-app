...
import { credibility, MENTOR_FLAG_WEIGHTS, linkedinNameMatch } from "./mentor-verification";
...
export function parseMentee(titles, item) {
  ...
  return {
    ...
    // Optional, and explicitly promised not to affect the application. Anything
    // here is a commitment the program has made to plan around, so it is
    // surfaced in the admin rather than scored: see the Access note block in
    // admin-fall.js. Never feeds eligibility, matching, or any rubric.
    accessNeeds: get("hard for you to take part"),
...
export function screenMentor(m, { returning = false, cutoff = MENTOR_FALL_CUTOFF } = {}) {
  ...
  // A separate failure from the one above. Sid Nag's URL has a malformed
  // domain but the slug is genuinely his, so these must not be merged.
  const liMatch = linkedinNameMatch(m.name, li);
  if (liMatch.checked && !liMatch.ok) add("linkedin-name-mismatch", `/in/${liMatch.slug}`);
...
export function parseMentor(titles, item) {
  ...
  const mentor = {
    ...
    // Same optional question as the founder form, same promise attached: it is
    // surfaced for a human and never reaches screenMentor or the rubric.
    accessNeeds: get("hard for you to take part"),
    assignedTo,
  };

...
export function parseMentor(titles, item) {
  ...
  const assignedTo = assigned.map(m => `${m.first} ${m.last}`.trim());
  ...
  // "assigned" to test accounts — flag it the same way test mentees are. A
  // mentor assigned to an actual founder is emphatically not a test account.
  const isTest = assigned.length > 0 && assigned.every(m => TEST_SLUGS.includes(m.slug));
  return {
    id: item.response_id,
    submittedAt: item.submitted_at,

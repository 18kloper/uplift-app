...
function profileCard(role, data, isMentor) {
  ...
  const BASE = "https://uplift-app-mocha.vercel.app";
  const resolvedPhoto = data.photo
    ? (data.photo.startsWith("http") ? data.photo : `${BASE}${data.photo}`)
    : "";

  const avatarInner = resolvedPhoto && isMentor
    ...
    : resolvedPhoto
      ? `<img src="${esc(resolvedPhoto)}" alt="${esc(data.name)}" width="72" height="72" style="width:72px;height:72px;border-radius:36px;display:block;object-fit:cover;border:2px solid #d4d0e8">`
      : isMentor
        ? `<div style="width:72px;height:72px;border-radius:36px;background:#1a1733;display:inline-block;text-align:center;line-height:72px;font-size:34px;border:2px solid #d4d0e8">🧑‍💼</div>`
